...
const client = process.env.GOOGLE_CX_KEY && process.env.GOOGLE_API_KEY
  ? new GoogleImages(
  ...
  function: {
    ...
    function: async ({ altText }: { altText: string[] }) => {
      try {
        if (!client) {
          return {
            error: "Google Image Search is not configured. Please set GOOGLE_CX_KEY and GOOGLE_API_KEY environment variables."
          };
        }
