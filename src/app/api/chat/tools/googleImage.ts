...
if (process.env.GOOGLE_CX_KEY && process.env.GOOGLE_API_KEY) {
    ...
    function: async ({ altText }: { altText: string[] }) => {
      if (!client) {
        return createToolErrorMessage(
          new Error("Google Images API credentials not configured"),
          {
            action: "searching for images",
            userFriendlyContext: "Google API credentials are missing",
            suggestion:
              "Please configure GOOGLE_CX_KEY and GOOGLE_API_KEY environment variables.",
          },
        );
      }
